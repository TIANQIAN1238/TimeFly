...
<div align="center">
  <em>TimeFly is a private, automatic timeline of your day.</em><br>
  Stop guessing where your time went. TimeFly watches your screen and builds a detailed timeline of your day. Private and open‑source. Nothing leaves your Mac.
</div>
...
<div align="center">
  <img src="docs/images/hero_animation_1080p.gif" alt="TimeFly Hero Animation" width="800">
</div>

<div align="center">
  <a href="https://github.com/JerryZLiu/TimeFly/releases/latest">
    <img src="https://img.shields.io/badge/Download%20for%20Mac-⬇%20%20TimeFly.dmg-blue?style=for-the-badge&logo=apple" alt="Download for Mac">
  </a>
</div>

<p align="center">
  <a href="#quickstart">Quickstart</a> •
  <a href="#why-timefly">Why TimeFly</a> •
...
TimeFly is a **native macOS app** (SwiftUI) that watches your screen, analyzes it with AI, and generates a **timeline** of your activities with summaries.
...
Most time trackers only log app usage. TimeFly understands context - the difference between "researching on YouTube" and "watching cat videos" - so your timeline reflects real work, not just which window was open.
...
<div align="center">
  <img src="docs/images/JournalPreview.png" alt="TimeFly journal preview" width="800">
</div>
...
- **Dashboard (beta - ChatGPT/Claude only)** - ask any question about your workday, pipe the answers into tiles you arrange yourself, and track trends over time.
  ...
  <div align="center">
    <img src="docs/images/DashboardPreview.png" alt="TimeFly dashboard preview" width="800">
  </div>
...
**Download (end users)**
1. Grab the latest `TimeFly.dmg` from **GitHub Releases**.
2. Open the app; grant **Screen & System Audio Recording** when prompted:  
   macOS → **System Settings** → **Privacy & Security** → **Screen & System Audio Recording** → enable **TimeFly**.

<div align="center">
  <a href="https://github.com/JerryZLiu/TimeFly/releases/latest">
    <img src="https://img.shields.io/badge/Download%20for%20Mac-⬇%20%20TimeFly.dmg-blue?style=for-the-badge&logo=apple" alt="Download the latest TimeFly.dmg">
  </a>
</div>

**Build from source (developers)**
1. Install **Xcode 15+** and open `TimeFly.xcodeproj`.
2. Run the `TimeFly` scheme on macOS 13+.
...
### From Releases
1. Download `TimeFly.dmg` and drag **TimeFly** into **Applications**.
...
### From source
```bash
git clone https://github.com/JerryZLiu/TimeFly.git
cd TimeFly
open TimeFly.xcodeproj
# In Xcode: select the TimeFly target, configure signing if needed, then Run.
```

### Homebrew

If you are using [Homebrew](https://brew.sh/), you can install [TimeFly](https://formulae.brew.sh/cask/timefly) with:

```bash
$ brew install --cask timefly
```
...
This section explains **what TimeFly stores locally**, **what leaves your machine**, and **how provider choices affect privacy**.

### Data locations (on your Mac)

All TimeFly data is stored in:
`~/Library/Application Support/TimeFly/`

- **Recordings (video chunks):** `TimeFly/recordings/` (or choose "Open Recordings..." from the TimeFly Taskbar Icon Menu)
- **Local database:** `TimeFly/chunks.sqlite`
- **Recording details:** Lightweight screen capture, analyzed periodically, configurable storage limits
- **Purge / reset tip:** Quit TimeFly. Then delete the entire `TimeFly/` folder to remove recordings and analysis artifacts. Relaunch to start fresh.

### Processing modes & providers
- **Gemini (cloud, BYO key)** - TimeFly sends batch payloads to **Google's Gemini API** for analysis.
- **Local models (Ollama / LM Studio)** - Processing stays **on‑device**; TimeFly talks to a **local server** you run.
- **ChatGPT / Claude (CLI-based, paid plan required)** - TimeFly drives the **Codex CLI** (ChatGPT) or **Claude Code CLI** directly on your Mac. **Requires an active ChatGPT Plus/Pro or Claude Pro subscription.** Uses frontier reasoning models for best-in-class narrative quality.
...
### Permissions (macOS)
To record your screen, TimeFly requires the **Screen & System Audio Recording** permission. Review or change later at:  
...
TimeFly registers a `timefly://` URL scheme so you can trigger common actions from Shortcuts, hotkey launchers, or scripts.

**Supported URLs**
- `timefly://start-recording` - enable capture (no-op if already recording)
- `timefly://stop-recording` - pause capture (no-op if already paused)

**Quick checks**
- From Terminal: `open timefly://start-recording` or `open timefly://stop-recording`
...
You can click the TimeFly icon in the menu bar and view the saved recordings

## Project structure

```
TimeFly/
├─ TimeFly/                 # SwiftUI app sources (timeline UI, debug UI, capture & analysis pipeline)
...
- **Screen capture is blank or fails**  
  Check System Settings → Privacy & Security → **Screen & System Audio Recording** and ensure **TimeFly** is enabled.
- **API errors**  
