        ...
        case .modelDownload(let command):
            VStack(alignment: .leading, spacing: 24) {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    Text("This model enables TimeFly to understand what's on your screen")
                        .font(.custom("Nunito", size: 14))
                        .foregroundColor(.black.opacity(0.6))
                }
        ...
        case .apiKeyInstructions:
            VStack(alignment: .leading, spacing: 24) {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    Text("Google's Gemini offers a generous free tier that should allow you to run TimeFly ~15 hours a day for free - no credit card required")
                        .font(.custom("Nunito", size: 14))
                        .foregroundColor(.black.opacity(0.6))
                }
    ...
    func configureSteps(for provider: String) {
        ...
        case "ollama":
            steps = [
                ...
                SetupStep(id: "complete", title: "Complete", contentType: .information("All set!", "Local AI is configured and ready to use with TimeFly."))
            ]
        case "chatgpt_claude":
            ...
            steps = [
                SetupStep(
                    ...
                    contentType: .information(
                        ...
                        "If you have a paid ChatGPT/Claude account, you can have TimeFly tap into your existing usage limits. Everything flows through your current account - no extra charges - and you can opt out of training for privacy. You only need one CLI installed and signed in on this Mac; we'll verify it automatically next."
                    )
                ),
                SetupStep(
                    id: "detect",
        ...
        default: // gemini
            steps = [
                ...
                SetupStep(id: "complete", title: "Complete",
                          contentType: .information("All set!", "Gemini is now configured and ready to use with TimeFly."))
            ]
        }
    }
...
struct ChatCLIDetectionStepView<NextButton: View>: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 24) {
            Text("TimeFly can talk to ChatGPT (via the Codex CLI) or Claude Code. You only need one installed and signed in on this Mac. After installing, run `codex auth` or `claude login` in Terminal to connect it to your account.")
                .font(.custom("Nunito", size: 14))
                .foregroundColor(.black.opacity(0.6))
            ...
            Text("Tip: Once both are installed, you can choose which provider TimeFly uses from Settings → AI Provider.")
                .font(.custom("Nunito", size: 12))
                .foregroundColor(.black.opacity(0.5))

            VStack(alignment: .leading, spacing: 10) {
                Text("Choose which provider TimeFly should use")
                    .font(.custom("Nunito", size: 13))
                    .fontWeight(.semibold)
                    .foregroundColor(.black.opacity(0.65))
                HStack(spacing: 12) {
