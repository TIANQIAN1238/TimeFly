...
struct HowItWorksView: View {
    ...
    private let fullText = "How TimeFly Works"
    ...
    private let cards: [(icon: String, title: String, body: String)] = [
        ("OnboardingHow",
         "Install and Forget",
         "TimeFly takes periodic screen captures to understand what you're working on, all stored privately on your device. You can toggle this whenever you like."),
        ("OnboardingSecurity",
         "Privacy by Default",
         "TimeFly can run entirely on local AI models, which means your data never leaves your computer. You can also find the source code below - please consider giving it a star on Github!"),
        ("OnboardingUnderstanding",
         "Understand your Day",
         "Knows the difference between YouTube tutorials and YouTube rabbit holes. TimeFly actually gets what you're working on.")
    ]
    ...
    var body: some View {
        ScrollView(.vertical, showsIndicators: false) {
                ...
                HStack {
                    ...
                    TimeFlySurfaceButton(
                        action: { if let url = URL(string: "https://github.com/jerryzliu/TimeFly") { NSWorkspace.shared.open(url) } },
                        content: {
                            HStack(spacing: 12) {
                                Image("GithubIcon").resizable().aspectRatio(contentMode: .fit).frame(width: 20, height: 20).colorInvert()
                                Text("Star TimeFly on GitHub").font(.custom("Nunito", size: 14)).fontWeight(.medium)
                            }
                        },
