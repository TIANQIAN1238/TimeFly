...
struct ActivityCard: View {
                ...
                if hasAnyActivities {
                    ...
                    if appState.isRecording {
                        VStack(spacing: 6) {
                            ...
                            Text("Cards are generated about every 15 minutes. If TimeFly is on and no cards show up within 30 minutes, please report a bug.")
                        ...
                        VStack(spacing: 6) {
                            Text("Recording is off")
                                .font(.system(size: 14, weight: .semibold))
                                .foregroundColor(.gray.opacity(0.7))
                            Text("TimeFly recording is currently turned off, so cards aren’t being produced.")
                                .font(.custom("Nunito", size: 13))
                                .foregroundColor(.gray.opacity(0.6))
                                .multilineTextAlignment(.center)
                                .padding(.horizontal, 16)
    ...
    private func activityDetails(for activity: TimelineActivity) -> some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            // Timelapse thumbnail (video pipeline)
