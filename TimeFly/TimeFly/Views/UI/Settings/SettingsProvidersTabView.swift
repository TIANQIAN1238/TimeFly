...
struct SettingsProvidersTabView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 28) {
                ...
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    default:
                        VStack(alignment: .leading, spacing: 8) {
                            Text("TimeFly Pro diagnostics coming soon")
                                .font(.custom("Nunito", size: 13))
            ...
            if viewModel.currentProvider == "gemini" {
                SettingsCard(title: "Gemini model preference", subtitle: "Choose which Gemini model TimeFly should prioritize") {
                ...
                SettingsCard(title: "Gemini prompt customization", subtitle: "Override TimeFly's defaults to tailor card generation") {
                ...
                SettingsCard(title: "ChatGPT / Claude prompt customization", subtitle: "Override TimeFly's defaults to tailor card generation") {
    ...
    private var geminiPromptCustomizationView: some View {
        VStack(alignment: .leading, spacing: 22) {
            Text("Overrides apply only when their toggle is on. Unchecked sections fall back to TimeFly's defaults.")
                .font(.custom("Nunito", size: 12))
                .foregroundColor(.black.opacity(0.55))
                .fixedSize(horizontal: false, vertical: true)
                ...
                TimeFlySurfaceButton(
                    ...
                    content: {
                        HStack(spacing: 8) {
                            ...
                            Text("Reset to TimeFly defaults")
                                .font(.custom("Nunito", size: 13))
                        }
    ...
    var body: some View {
        ScrollView(.vertical, showsIndicators: true) {
            VStack(alignment: .leading, spacing: 24) {
                HStack {
                    VStack(alignment: .leading, spacing: 6) {
                        Text("Upgrade to \(preset.displayName)")
                            ...
                            .fontWeight(.semibold)
                        Text("Follow the steps below, run a quick test, and TimeFly will switch you over automatically.")
                            .font(.custom("Nunito", size: 13))
                            .foregroundColor(.black.opacity(0.6))
                    }
                    Spacer()
                ...
                Text("Once the test succeeds, TimeFly updates your settings to \(preset.displayName) automatically.")
                    .font(.custom("Nunito", size: 12))
                    .foregroundColor(.black.opacity(0.55))
...
private struct GeminiModelSettingsCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            Text("TimeFly automatically downgrades if your chosen model is rate limited or unavailable.")
