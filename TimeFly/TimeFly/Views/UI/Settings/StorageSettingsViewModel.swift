...
final class StorageSettingsViewModel: ObservableObject {
    ...
    func storageFooterText() -> String {
        ...
        return "Recording cap: \(recordingsText) • Timelapse cap: \(timelapsesText). Lowering a cap immediately deletes the oldest files for that type. Timeline card text stays preserved. Please avoid deleting files manually so you do not remove TimeFly's database."
    }
