...
final class StorageManager: StorageManaging, @unchecked Sendable {
    ...
    private let dbWriteQueue = DispatchQueue(label: "com.timefly.storage.writes", qos: .utility)
    ...
    private init() {
        ...
        let baseDir = appSupport.appendingPathComponent("TimeFly", isDirectory: true)
    ...
    /// Creates a dummy "Installed TimeFly!" card when onboarding completes.
    ...
    func createOnboardingCard() {
        ...
        let meta = TimelineMetadata(
            distractions: nil,
            appSites: AppSites(primary: "timefly.so", secondary: nil),
    ...
    private func buildOnboardingSummary() -> String {
        ...
        case "gemini":
            return "You successfully installed TimeFly and configured it with Gemini AI. Come back in 30 minutes to see your first real activity card! ✨ (This is a sample card, so you can see what your timeline will look like.)"

        case "chatgpt_claude":
            // Check which CLI tool they picked
            let cliTool = UserDefaults.standard.string(forKey: "chatCLIPreferredTool") ?? "claude"
            if cliTool == "codex" {
                return "You successfully installed TimeFly with ChatGPT. Come back in 30 minutes to see your first real activity card! ✨ (This is a sample card, so you can see what your timeline will look like.)"
            } else {
                return "You successfully installed TimeFly with Claude. Come back in 30 minutes to see your first real activity card! ✨ (This is a sample card, so you can see what your timeline will look like.)"
            }
        ...
        case "ollama":
            ...
            if localEngine == "lmstudio" {
                return "You successfully installed TimeFly with LM Studio — your data stays 100% on your device. Come back in 30 minutes to see your first real activity card! ✨ (This is a sample card, so you can see what your timeline will look like.)"
            } else {
                return "You successfully installed TimeFly with Ollama — your data stays 100% on your device. Come back in 30 minutes to see your first real activity card! ✨ (This is a sample card, so you can see what your timeline will look like.)"
            }

        default:
            return "You successfully installed TimeFly. Come back in 30 minutes to see your first real activity card! ✨ (This is a sample card, so you can see what your timeline will look like.)"
        }
    }
    ...
    private let purgeQ = DispatchQueue(label: "com.timefly.storage.purge", qos: .background)
...
private extension StorageManager {
    func migrateLegacyChunkPathsIfNeeded() {
        ...
        let legacyBase = fileMgr.homeDirectoryForCurrentUser
            .appendingPathComponent("Library/Containers/\(bundleID)/Data/Library/Application Support/TimeFly", isDirectory: true)
        let newBase = appSupport.appendingPathComponent("TimeFly", isDirectory: true)
