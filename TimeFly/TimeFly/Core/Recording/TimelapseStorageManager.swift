...
final class TimelapseStorageManager {
    ...
    private let queue = DispatchQueue(label: "com.timefly.timelapse.purge", qos: .utility)

    private init() {
        let appSupport = fileMgr.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
        let path = appSupport.appendingPathComponent("TimeFly/timelapses", isDirectory: true)
        root = path
