...
struct StatusMenuView: View {
    ...
    var body: some View {
        VStack(spacing: 6) {
            ...
            MenuRow(title: "Open TimeFly", systemImage: "macwindow", action: openTimeFly)
            MenuRow(title: "Open Recordings", action: openRecordingsFolder)
            MenuRow(title: "Check for Updates", action: checkForUpdates)

            MenuDivider()

            MenuRow(title: "Quit Completely", systemImage: "power", accent: .red, action: quitTimeFly)
        }
        .padding(.vertical, 9)
