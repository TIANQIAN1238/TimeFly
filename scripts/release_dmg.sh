...
# Release helper for TimeFly: builds, signs, notarizes, and packages a DMG for distribution.
#
# Usage:
#   ./scripts/release_dmg.sh
#
# Optional env vars:
#   SCHEME           - Xcode scheme (default: TimeFly)
#   CONFIG           - Xcode configuration (default: Release)
#   DERIVED_DATA     - Derived data path (default: build)
#   APP_NAME         - App name (default: TimeFly)
#   ENTITLEMENTS     - Entitlements plist path (default: TimeFly/TimeFly/TimeFly.entitlements)
...
SCHEME=${SCHEME:-TimeFly}
CONFIG=${CONFIG:-Release}
DERIVED_DATA=${DERIVED_DATA:-build}
APP_NAME=${APP_NAME:-TimeFly}
ENTITLEMENTS=${ENTITLEMENTS:-TimeFly/TimeFly/TimeFly.entitlements}
...
if command -v rg >/dev/null 2>&1; then
  ...
  if ! printf "%s" "$ENT_DUMP" | rg -q "-spks|teleportlabs.com.TimeFly-spks"; then
    echo "WARNING: Sparkle status mach service (-spks) not present in entitlements." >&2
  fi
  if ! printf "%s" "$ENT_DUMP" | rg -q "-spki|teleportlabs.com.TimeFly-spki"; then
...
# TimeFly app on left, Applications folder on right (swapped from typical layout)
create-dmg \
  --volname "${VOL_NAME}" \
  --background "${DMG_BG}" \
  --window-size 775 480 \
